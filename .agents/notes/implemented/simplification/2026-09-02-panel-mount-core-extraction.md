...
The takeover lifecycle now lives exactly once in `shared/client/panel-mount-core.ts`: `mountCenterPanel(options)` owns the column selector, the MutationObserver remount, the eviction-and-activate sequence, the sidebar click-out listener, and the disposer order; the `CenterPanelMountOptions` contract carries the seven per-plugin parameters plus the controller subscribe and the optional locale source. The file joins the sync-shared manifest with one generated copy (`packages/dsh-ssh/src/client/panel-mount-core.ts`), the only consumer left after task-board moved to the native layout seats (below); the sync-shared test's copy-count buckets are 98 total and 38 client. dsh-ssh's wrapper stays the parameter wiring and keeps its public exports (`mountPanel` + `PANEL_VIEW_SELECTOR`). The rebuilt aggregate client bundle (`packages/dsh-web-all/lib/client.js`) ships the same behavior inlined from source.

Container attribute names stay wrapper-supplied parameters: they are pinned by each package's CSS (`panel.module.css` cross-references the sibling's html attribute), the wallpaper-exclusive skin patches, and the semantic-attributes contract, so the extraction deliberately changes none of them.

## Later change: task-board left the takeover for the native layout seats

The task board no longer consumes this core. It contributes a `sidebar.panellist` row and a keyed `main` page through the official slots system and drives `ctx.layout.selectPanel`, so the shell owns the center column and the board renders in the same container the desktop application's Plugins page uses; the DOM takeover, its self-healing observer, and the `html[data-dsh-taskboard-active]` visibility contract are gone from the package. See [task-board native panel adoption](../../architecture/2026-09-25-task-board-native-panel-and-timer.md).

Cross-plugin exclusivity did **not** go with it: dsh-ssh still takes the column over at the DOM level, and while `html[data-dsh-ssh-active]` is set its stylesheet hides every other column child — the board's page included. The board therefore keeps participating in the shared `dsh-panel-activate` protocol (`PANEL_ACTIVATE_EVENT`, detail `taskboard`/`ssh`) defined here, in `packages/dsh-task-board/src/client/native-panel.tsx`: opening the board announces `taskboard` so ssh releases the column, and an incoming `ssh` announcement closes the board. This is the one behavior this core still shares with the board, and it is the reason the event name and detail values remain a contract rather than a private detail of the ssh copy.

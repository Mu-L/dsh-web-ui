...
- Deployment needs the worker route plus a wildcard DNS record for `*.t.dsh-market.com` before the relay works; until then the plugin degrades to today's raw quick URL behavior.

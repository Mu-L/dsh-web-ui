...
Host listing (`src/host/state.ts`): `buildPluginRow` emits `children` when the installed package's bundle patch claims more than one insert row. A child's display name prefers the real plugin package (the aggregate shell row's `config.plugin`, now parsed by `insertRowsOf`) over the per-family subpath name. Rows in `LOCKED_ENTRY_IDS` (the family settings surface, the aggregate compat face — standalone and aggregate spellings) carry `locked: true`.

Row-level writes (`src/host/routes.ts` set-enabled): the `id` payload first resolves as a profile dependency (whole-package toggle, existing semantics). Otherwise `findRowOwner` locates the dependency whose bundle patch claims that entry id, and only that row gets the `{ id, name, disabled }` user-layer override — the id-targeted, later-wins patch semantics the official desktop writer uses. Disabling a locked row is rejected with an error; a whole-package disable still force-keeps locked rows enabled, so the GUI escape hatch can never be switched off by its own hand. The response carries the refreshed OWNING package row (with children), so a caller patches its state by the returned row id.

Rendering moved to the official page: the package row's row list is the official plugin manager page's own, so the child granularity this decision created is surfaced by that page's per-row switches rather than by any UI this package draws. A locked row is still listed, without a usable switch.
...
- Verified: `pnpm --filter @linxin666/dsh-client-ui-plugin-manager test` (row-level toggle/sibling isolation, locked-row rejection, insert-format row edits, and wire parsing), typecheck, build, and `pnpm i18n:check` (zh/en/ru key parity).

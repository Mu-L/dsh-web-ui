...
The local web profile carries one out-of-tree patch; profile installs and plugin updates silently revert it. A future session seeing this pageerror should check this note and the backup file before re-deriving the attribution. The same-day investigation of a user-reported "Chrome repeatedly crashes after dsh web starts" found no Chrome crashes at any level — no Crashpad dumps, no macOS DiagnosticReports, no jetsam kills, clean Chrome stability counters; the perceived crash loop was the browser-automation daemon relaunching Chrome plus Chrome's remote-debugging consent relaunch during verification.
